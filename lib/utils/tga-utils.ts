  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const tga = new TGA(e.target.result as ArrayBuffer);
        const imageData = tga.getImageData();
        
        if (!imageData || !imageData.data || imageData.width <= 0 || imageData.height <= 0) {
          reject(new Error('Invalid TGA data'));
          return;
        }
        
        resolve(imageData);
      } catch (error) {
        reject(new Error(`Failed to decode TGA: ${error}`));
      }
    };
 ...
 */
export async function imageDataToBlobURL(imageData: TGAImageData): Promise<string> {
...
export function detectImageSequence(files: File[]): ImageSequence | null {
  ...
  console.log('[TGA Utils] Checking for sequence in files:', imageFiles.map(f => f.name));
  ...
  for (const pattern of patterns) {
    ...
    if (matches.length >= 2) {
      console.log('[TGA Utils] Found pattern matches:', matches.map(m => ({ name: m!.file.name, frame: m!.frameNumber })));
      
      // Check if frame numbers are consecutive or have a pattern
      const frameNumbers = matches.map(m => m!.frameNumber).sort((a, b) => a - b);
      const isConsecutive = frameNumbers.every((num, i) => 
        i === 0 || num === frameNumbers[i - 1] + 1
      );
 ...
 */
export async function processTGAFiles(files: File[]): Promise<{ file: File; blobUrl: string }[]> {
  const results: { file: File; blobUrl: string }[] = [];
  
  console.log('[TGA Utils] Processing files:', files.map(f => ({ name: f.name, type: f.type, size: f.size })));
  ...
  for (const file of files) {
    if (file.name.toLowerCase().endsWith('.tga')) {
      ...
      try {
        ...
        console.log(`[TGA Utils] TGA decoded successfully: ${file.name} -> ${imageData.width}x${imageData.height}`);
